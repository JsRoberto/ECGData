...
Nessa fase, o sinal passa por um bloco de filtros para reduzir ruídos e influências da onda T. Esse bloco de filtros é composto por um passa-baixa e um passa-alta em cascata para construir um passa-banda de 3dB de atenuação e frequências de 9 a 20 Hz[ref. ALVAREZ]. O passa-baixa é usado para remover ruídos de altas frequências e o passa-alta é usado para atenuar as ondas P e T e o "baseline drift". O filtro digital passa-banda aumenta a relação sinal-ruido (SNR).
Em seguida, um operador derivada é aplicado para filtrar a saída, fornecendo informações da inclinação complexas. Então, o sinal é elevado ao quadrado ponto a ponto, o que intensifica a inclinação do sinal derivado e ajuda a reduzir falsos positivos causados por de ondas T maiores que as usuais.
Finalmente, uma janela de integração móvel é aplicada, o que agraga informações sobre inclinação e largura do sinal. O tamanho da janela de integração é muito importe, uma vez que deve sempre conter o complexo QRS que pode ser de diferentes extensões e, entretanto, não deve misturado com outras ondas como a onda T. O melhor resultado foi obtido usando uma janela de 150ms, além disso os filtros passa-banda e as operações derivativas causam um delay que devem ser considerados quando comparados ao sinal de referência.
