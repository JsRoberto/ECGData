...
Em processamento de eletrocardiogramas (ECG), uma base importante para fundamentar posteriores análises dos dados é a precisão na detecção de batimentos cardíacos. Nesse sentido, como a energia dos batimentos está localizada nos complexos QRS, o foco na detecção desses complexos é essencial para a análise de ECG.  
...
O algoritmo de detecção do complexo QRS a ser estudado e aplicado será o algoritmo de _Pan & Tompkins_, cujas etapas de processamento serão abordadas mais adiante. A discussão inclui a análise em pormenor do código fonte desenvolvido neste trabalho e que utilizou parâmetros previamente estabelecidos em literatura sobre o tema [ÁLVAREZ][1].  

O algoritmo de _Pan & Topmpkins_ será aplicado sobre uma base de dados pública, o bando de dados [Physionet](http://www.physionet.org/physiobank/database/#ecg). Mais especificamente, os dados que utilizaremos foram coletados e armazenados pelo _MIT_ e pelo _Boston’s Beth Israel Hospital_ (_MIT-BIH Arrhythmia Database_) segundo metodologia amplamente submetida a discussões e críticas, o que colaborou para manter sua relevância acadêmica. Além disso, essa base de dados é focada em sinais com as mais diversas morfologias de complexos QRS,  o que permite investigar a performance dos algoritmos de detecção desses complexos – daí, portanto, seu extensivo uso para validação desses algoritmos.  

O _MIT-BIH Arrhythmia Database_ contém 48 horas e meia de gravações coletadas utilizando-se dois canais de entrada. Todavia, o código fonte que desenvolvemos processará somente um desses canais: se um desses canais registrar sinais de alta qualidade, normalmente isso implica que o segundo canal registrará sinais de baixa amplitude e de baixa relação sinal-ruído.  

Os softwares utilizados serão:   
(1) _MatLab_, da _Mathworks_ – utilizar-se-á o WFDB Toolbox apara baixar os dados do _Physionet_; 
(2) _RStudio_, linguagem de programação R – utilizar-se-á esta linguagem para o processamento dos sinais de ECG do MIT-BIH. Seus principais atrativos são seus ferramentais voltados para computação estatística e modelagem gráfica. Além disso, o software é _open source_ e apresenta estrutura multiplataforma.  
...
Esta etapa consiste em (1) estabelecer as bibliotecas a serem utilizadas dentro do código, entre as quais temos os pacotes gráficos _ggplot2_ e _gridExtra_, além do pacote _signal_, com funções de filtros digitais diretamente importadas do MatLab e Octave [CRAN - package signal][2]; (2) baixar e salvar no diretório de trabalho os dois arquivos necessários ao processamento – *mtidb_signals.csv* e *fs.csv*; e (3) criar a principal função de plotagem gráfica _dataECGplot()_, que mostrará os resultados das principais etapas do processamento dos sinais.  

```{r setoptions}
knitr::opts_chunk$set(autodep = FALSE, cache = FALSE, warning = FALSE, results = "hide")
```
...
O método que _Pan & Tompkins_ desenvolveram para realizar a detecção dos complexos QRS de sinais de ECG é capaz de realizar a tarefa em tempo. Para tanto, o algoritmo é baseado na inclinação, amplitude e tamanho do sinal - não se limita, pois, somente à inclinação da onda R do sinal.  
...
Nessa fase, o sinal passa por um bloco de filtros para reduzir ruídos e influência das ondas T. Esse bloco de filtros é composto por um passa-baixa e um passa-alta em cascata, com o intuito de construir um passa-banda entre 9-20 Hz com 3 dB de atenuação [RANGAYYAN][3]. O passa-baixa é usado para remover ruídos de altas frequências e o passa-alta é usado para atenuar as ondas P e T e o _baseline drift_ do sinal. O filtro digital passa-banda aumenta a relação sinal-ruido (SNR).  
...
Finalmente, uma janela de integração móvel é aplicada, o que agrega informações sobre inclinação e largura do sinal. O tamanho da janela de integração é muito importante, uma vez que deve sempre conter o complexo QRS.  

As dificuldades são as seguintes: (1) o complexo QRS pode apresentar diferentes extensões e (2) o tamanho da janela não deve excedê-lo, porquanto não deve se misturar a outras formas de onda, como a onda T. A janela utilizada apresenteu os melhores resultados, segundo [ÁLVAREZ][1], com uma janela de 150 ms - isso corresponde a `r 0.15*fs` amostras para os dados utilizados neste trabalho.  

Além disso, cabe ressaltar que todas as etapas de processamento, seja a etapa dos filtros que constituem o passa-banda, as operações derivativas ou a janela de integração móvel, todas elas causam _delays_ que devem ser considerados quando comparados os sinais resultantes de cada etapa.  
...
Em seguida, apresentamos os códigos que executam as funções anteriormente apresentadas.
...
O mesmo procedimento que terminamos de aplicar sobre _dt.signal_, também precisamos aplicar sobre _mwi.signal_ para concluir a última parte da fase de decisão do algoritmos de Pan & Tompkins.
...
A aplicação do algoritmo de decisão de _Pan & Tompkins_ sobre ambas as listas _dt.signal_ e _mwi.signal_ é importante para a último bloco de processamento do método: a comparação dos dados obtidos a partir das duas listas, começando por (1) considerar o _delay_ entre o sinal derivativo e o sinal pós-mwi (mwi = *moving window integration*), que corresponde à metade da janela, ou seja, `r D_if/2`; (2) estabelecer como teste para classificar uma detecção verdadeira a proximidade menor que 50 amostras entre as detecções análogas dos dois conjuntos de sinais.  

Isso permitirá um aumento na confiabilidade da detecção se comparado à detecção feita com apenas um deles, _dt.signal_ ou _mwi.signal_. Cabe ressaltar, contudo, que não executamos no nosso trabalho esta última etapa do processamento, limitando-nos a utilizar os resultados que o algoritmo de classificação obteve quando aplicado sobre _dt.signal_.  

Então, os dados assim obtidos estão representados nas tabelas a seguir.  

Caso|Média e dp|Média e dp após norm.|Nº QRS verdadeiro(Nv)|Nº QRS detectado(Nd)
----|----------|---------------------|---------------------|--------------------
102|`r round(Mean1[1],2)` (`r round(Std1[1],2)`)|`r round(Mean2[1],2)` (`r round(Std2[1],2)`)|`r length(dt.signalSPeaks[[1]])`|`r length(dt.signalSPeaks[[1]]) + dt.signalFalsePos[[1]]`
103|`r round(Mean1[2],2)` (`r round(Std1[2],2)`)|`r round(Mean2[2],2)` (`r round(Std2[2],2)`)|`r length(dt.signalSPeaks[[2]])`|`r length(dt.signalSPeaks[[2]]) + dt.signalFalsePos[[2]]`|`r dt.signalFalsePos[[2]]`
104|`r round(Mean1[3],2)` (`r round(Std1[3],2)`)|`r round(Mean2[3],2)` (`r round(Std2[3],2)`)|`r length(dt.signalSPeaks[[3]])`|`r length(dt.signalSPeaks[[3]]) + dt.signalFalsePos[[3]]`|`r dt.signalFalsePos[[3]]`
105|`r round(Mean1[4],2)` (`r round(Std1[4],2)`)|`r round(Mean2[4],2)` (`r round(Std2[4],2)`)|`r length(dt.signalSPeaks[[4]])`|`r length(dt.signalSPeaks[[4]]) + dt.signalFalsePos[[4]]`|`r dt.signalFalsePos[[4]]`
106|`r round(Mean1[5],2)` (`r round(Std1[5],2)`)|`r round(Mean2[5],2)` (`r round(Std2[5],2)`)|`r length(dt.signalSPeaks[[5]])`|`r length(dt.signalSPeaks[[5]]) + dt.signalFalsePos[[5]]`|`r dt.signalFalsePos[[5]]`
107|`r round(Mean1[6],2)` (`r round(Std1[6],2)`)|`r round(Mean2[6],2)` (`r round(Std2[6],2)`)|`r length(dt.signalSPeaks[[6]])`|`r length(dt.signalSPeaks[[6]]) + dt.signalFalsePos[[6]]`|`r dt.signalFalsePos[[6]]`

  
  
Caso|Nº Falso Positivo(FP)|%FP = 100×FP/Nv|Nº Falso Negativo(FN)|%FN = 100×FN/Nv
----|---------------------|---------------|---------------------|---------------
102|`r dt.signalFalsePos[[1]]`|`r signif(100*dt.signalFalsePos[[1]]/length(dt.signalSPeaks[[1]]),4)` %|`r dt.signalFalseNeg[[1]]`|`r signif(100*dt.signalFalseNeg[[1]]/length(dt.signalSPeaks[[1]]),4)` %
103|`r dt.signalFalsePos[[2]]`|`r signif(100*dt.signalFalsePos[[2]]/length(dt.signalSPeaks[[2]]),4)` %|`r dt.signalFalseNeg[[2]]`|`r signif(100*dt.signalFalseNeg[[2]]/length(dt.signalSPeaks[[2]]),4)` %
104|`r dt.signalFalsePos[[3]]`|`r signif(100*dt.signalFalsePos[[3]]/length(dt.signalSPeaks[[3]]),4)` %|`r dt.signalFalseNeg[[3]]`|`r signif(100*dt.signalFalseNeg[[3]]/length(dt.signalSPeaks[[3]]),4)` %
105|`r dt.signalFalsePos[[4]]`|`r signif(100*dt.signalFalsePos[[4]]/length(dt.signalSPeaks[[4]]),4)` %|`r dt.signalFalseNeg[[4]]`|`r signif(100*dt.signalFalseNeg[[4]]/length(dt.signalSPeaks[[4]]),4)` %
106|`r dt.signalFalsePos[[5]]`|`r signif(100*dt.signalFalsePos[[5]]/length(dt.signalSPeaks[[5]]),4)` %|`r dt.signalFalseNeg[[5]]`|`r signif(100*dt.signalFalseNeg[[5]]/length(dt.signalSPeaks[[5]]),4)` %
107|`r dt.signalFalsePos[[6]]`|`r signif(100*dt.signalFalsePos[[6]]/length(dt.signalSPeaks[[6]]),4)` %|`r dt.signalFalseNeg[[6]]`|`r signif(100*dt.signalFalseNeg[[6]]/length(dt.signalSPeaks[[6]]),4)` %

  
  
Caso|Média do IRR|Dp IRR
----|------------|------
102|`r round(mean(dt.signalRRinterval[[1]]/fs),2)`|`r round(sd(dt.signalRRinterval[[1]]/fs),2)`
103|`r round(mean(dt.signalRRinterval[[2]]/fs),2)`|`r round(sd(dt.signalRRinterval[[2]]/fs),2)`
104|`r round(mean(dt.signalRRinterval[[3]]/fs),2)`|`r round(sd(dt.signalRRinterval[[3]]/fs),2)`
105|`r round(mean(dt.signalRRinterval[[4]]/fs),2)`|`r round(sd(dt.signalRRinterval[[4]]/fs),2)`
106|`r round(mean(dt.signalRRinterval[[5]]/fs),2)`|`r round(sd(dt.signalRRinterval[[5]]/fs),2)`
107|`r round(mean(dt.signalRRinterval[[6]]/fs),2)`|`r round(sd(dt.signalRRinterval[[6]]/fs),2)`

***
[1]: http://www.esei.uvigo.es/fileadmin/docs/estudiantes/Traballos_fin_de_carreira/TFM_SSIA_2012-13/A_comparison_of_three_QRS_detection_algoritms_over_a_public_database.pdf "ÁLVAREZ"

[2]: https://cran.r-project.org/web/packages/signal/signal.pdf "CRAN - package signal"

[3]: http://gen.lib.rus.ec/book/index.php?md5=E3D014CA2546BED013AF02453EC25CC9 "RANGAYYAN"
