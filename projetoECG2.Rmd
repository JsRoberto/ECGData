...
Uma vez que o sinal é pré-processado, ele está preparado para a detecção dos complexo QRS. Nesse estágio dois conjuntos de limiares são aplicados aos dois, ECG filtrado e o sinal depois da janela de integração. Por usar limiares nos dois sinais, a confiabilidade da detecção está na comparação.  

O limiares de detecção variam de acordo com o ruído que é detectado pelo algoritmo, ajustando-os ao sinal e mudando automaticamente as condições. Para essa realização, primeiro é necessário uma detecção dos picos, considerando que os picos é um máximo local, todos os picos devem ser classificados como ruído ou sinal, de acordo com o limiar estabelecido.  

Foi desenvolvido um algoritmo de detecção de máximos locais, definido como pico o valor mais alto em um intervalo, nesta aplicação intervalo foi definido como 80ms. Um intervalo menor detectaria muitos picos, enquanto que um grande intervalo poderia não detectar alguns dele.
